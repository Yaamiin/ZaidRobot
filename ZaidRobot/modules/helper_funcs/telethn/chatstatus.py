from ZaidRobot.modules.helper_funcs.telethn import IMMUNE_USERS, telethn
from ZaidRobot import DRAGONS
...
async def zaid_is_admin(chat_id: int):
    status = False
    zaid = await telethn.get_me()
    async for user in telethn.iter_participants(
        chat_id, filter=ChannelParticipantsAdmins
    ):
        if zaid.id == user.id:
